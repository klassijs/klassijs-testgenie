...
} = require("../services/zephyrService");
const {
  testJiraConnection,
  getJiraProjects,
  getJiraIssues,
...
} = require("../services/localStorageCRUD");
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (isSupported) {
      ...
      cb(
        new Error(
          `Invalid file type. Only PDF, DOCX, DOC, TXT, MD, RTF, ODT, images (JPG, PNG, GIF, etc.), Excel (XLS, XLSX), PowerPoint (PPT, PPTX), and Visio (VSD, VSDX) files are allowed.`,
        ),
        false,
      );
    }
...
router.get("/loading-images", (req, res) => {
  try {
    const loadingImagesPath = path.join(
      __dirname,
      "../../frontend/public/images/loading",
    );
  ...
  try {
    ...
    if (!content || content.trim().length < 10) {
      return res.status(400).json({
        ...
        details:
          "The uploaded file could not be processed or contains insufficient content",
  ...
  try {
    ...
    if (content.trim().length < 10) {
      return res.status(400).json({
        ...
        details:
          "The provided content is too short to generate meaningful test cases",
    ...
    if (error.message.includes("Azure OpenAI is not configured")) {
      ...
      suggestion =
        "Please configure Azure OpenAI credentials to generate test cases";
    } else if (
      error.message.includes(
        "Content was filtered by Azure OpenAI safety filters",
      )
    ) {
      errorMessage = "Content flagged by safety filters";
      suggestion =
        "The document contains content that was flagged by AI safety filters. Please try uploading a different document or contact your administrator if you believe this is an error.";
      statusCode = 400;
    } else if (
      ...
      suggestion =
        "Too many requests to the AI service. Please wait a few minutes and try again. The system will automatically retry with delays.";
      statusCode = 429;
    } else if (
      error.message.includes("No content received from Azure OpenAI")
    ) {
      errorMessage = "No response from AI service";
      suggestion =
        "The AI service did not return any content. Please try again or contact support if the issue persists.";
    }
...
router.post("/refine-tests", async (req, res) => {
  try {
    ...
    if (error.message.includes("Azure OpenAI is not configured")) {
      ...
      suggestion =
        "Please configure Azure OpenAI credentials to refine test cases";
    } else if (
      error.message.includes(
        "Content was filtered by Azure OpenAI safety filters",
      )
      ...
      errorMessage = "Content flagged by safety filters";
      suggestion =
        "The test content contains material that was flagged by AI safety filters. Please try refining with different content or contact your administrator if you believe this is an error.";
      statusCode = 400;
    } else if (
      error.message.includes("No content received from Azure OpenAI")
    ) {
  ...
  try {
    ...
    if (content.trim().length < 50) {
      return res.status(400).json({
        error: "Insufficient content",
        details:
          "The provided content is too short to extract meaningful requirements",
        suggestion:
          "Please provide more detailed content for requirements extraction",
      });
    }

    const { enableLogging = true } = req.body;
    const extractedRequirements = await extractBusinessRequirements(
      content,
      context,
      enableLogging,
    );
    ...
    writeToStorage(extractedRequirements);
    ...
    if (error.message.includes("Azure OpenAI is not configured")) {
      ...
      suggestion =
        "Please configure Azure OpenAI credentials to extract requirements";
    } else if (
      error.message.includes(
        "Content was filtered by Azure OpenAI safety filters",
      )
      ...
      suggestion =
        "The document is very large. The system will process the first portion. For complete analysis of large documents, consider splitting into smaller files or uploading sections separately.";
      statusCode = 400;
    } else if (
      error.message.includes("No content received from Azure OpenAI")
    ) {
  ...
  try {
    if (!isZephyrConfigured) {
      return res.status(503).json({
        ...
        details:
          "Missing Zephyr Scale credentials. Please configure ZEPHYR_BASE_URL, ZEPHYR_API_TOKEN, and ZEPHYR_PROJECT_KEY in your .env file.",
    ...
    if (error.response) {
      ...
      suggestion =
        "Please check your Zephyr Scale credentials and project configuration";
...
router.get(
  "/zephyr-subfolders/:projectKey/:parentFolderId",
  async (req, res) => {
    try {
      ...
      if (!isZephyrConfigured) {
        return res.status(503).json({
          ...
          details:
            "Missing Zephyr Scale credentials. Please configure ZEPHYR_BASE_URL, ZEPHYR_API_TOKEN, and ZEPHYR_PROJECT_KEY in your .env file.",
          suggestion:
            "Set up Zephyr Scale credentials to enable folder listing",
        });
      }
      ...
      let suggestion =
        "Please check your Zephyr Scale credentials and try again";
      ...
      if (error.response) {
        ...
        errorMessage = "Network error connecting to Zephyr Scale";
        suggestion =
          "Please check your Zephyr Scale URL and network connection";
      }
    ...
    if (!isZephyrConfigured) {
      return res.status(503).json({
        ...
        details:
          "Missing Zephyr Scale credentials. Please configure ZEPHYR_BASE_URL, ZEPHYR_API_TOKEN, and ZEPHYR_PROJECT_KEY in your .env file.",
        suggestion:
          "Set up Zephyr Scale credentials to enable direct push functionality",
      });
    }

    const response = await pushToZephyr(
      content,
      featureName,
      projectKey,
      testCaseName,
    ...
    if (folderId) {
      try {
        const folderResponse = await axios.get(
          `${process.env.ZEPHYR_BASE_URL}/folders/${folderId}`,
          {
            headers: {
...
router.get("/zephyr/test-endpoints/:projectKey", async (req, res) => {
  try {
    ...
    const testEndpoints = [
      {
        name: "testcases",
        url: `${process.env.ZEPHYR_BASE_URL}/testcases`,
        method: "GET",
      },
      {
        name: "folders",
        url: `${process.env.ZEPHYR_BASE_URL}/folders`,
        method: "GET",
      },
      {
        name: "coverage",
        url: `${process.env.ZEPHYR_BASE_URL}/coverage`,
        method: "GET",
      },
      {
        name: "traceability",
        url: `${process.env.ZEPHYR_BASE_URL}/traceability`,
        method: "GET",
      },
      {
        name: "issues",
        url: `${process.env.ZEPHYR_BASE_URL}/issues`,
        method: "GET",
      },
      {
        name: "links",
        url: `${process.env.ZEPHYR_BASE_URL}/links`,
        method: "GET",
      },
      {
        name: "weblinks",
        url: `${process.env.ZEPHYR_BASE_URL}/weblinks`,
        method: "GET",
      },
      {
        name: "comments",
        url: `${process.env.ZEPHYR_BASE_URL}/comments`,
        method: "GET",
      },
    ];
  ...
  try {
    ...
    if (result.success) {
      ...
      res.status(400).json({
        ...
        details: result.error,
        suggestion:
          "Please check your Jira environment variables (JIRA_BASE_URL, JIRA_EMAIL, JIRA_API_TOKEN)",
      });
    }
  } catch (error) {
...
router.post("/analyze-business-elements", async (req, res) => {
  try {
    ...
    const {
      extractBusinessRequirements: universalExtract,
    } = require("../utils/universalBusinessExtractor");
