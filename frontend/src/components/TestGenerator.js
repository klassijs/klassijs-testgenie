...
const TestGenerator = () => {
  ...
  const exportToJira = (content) => {
    // Convert Gherkin to Jira format
    const lines = content.split('\n');
    let jiraContent = '';
    let currentFeature = '';
    let currentScenario = '';
    
    jiraContent += `{panel:title=Generated Test Cases|borderStyle=dashed|borderColor=#ccc|titleBGColor=#f0f0f0|bgColor=#f9f9f9}\n`;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('Feature:')) {
        ...
        jiraContent += `h1. ${currentFeature}\n\n`;
      } else if (line.startsWith('Scenario:')) {
        currentScenario = line.replace('Scenario:', '').trim();
        jiraContent += `h2. ${currentScenario}\n\n`;
        jiraContent += `{panel:title=Test Steps|borderStyle=dashed|borderColor=#ccc|titleBGColor=#e6f3ff|bgColor=#f8fbff}\n`;
    ...
    jiraContent += `{panel}\n\n`;
    jiraContent += `*Generated by AI Test Automation Platform*\n`;
    
    const blob = new Blob([jiraContent], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'jira-test-cases.txt';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    return jiraContent; // Return the content for clipboard
  };
  ...
  const pushToZephyr = async (content, featureName = 'Test Feature', projectKey = '', testCaseName = '', folderId = '') => {
    try {
            ...
            <div className="modal-footer">
              <div className="export-actions">
                ...
                <button
                  ...
                  onClick={() => {
                    const currentContent = editableFeatures[activeTab] || featureTabs[activeTab]?.content || '';
                    const jiraContent = exportToJira(currentContent);
                    navigator.clipboard.writeText(jiraContent);
                    setStatus({ type: 'success', message: 'Exported to Jira format and copied to clipboard!' });
                  }}
                  title="Export current feature to Jira format"
                >
